...
$query = "SELECT MAX(id) AS max_id FROM purchase";
$resultado = mysqli_query($conexion, $query);
$row = mysqli_fetch_assoc($resultado);
$max_id = $row['max_id'];
// Incrementar el valor en 1
$idCompraInicial = ($max_id == null) ? 1 : $max_id + 1;
$success = '';
...
if (!empty($_POST)) {
    ...
    $fecha = $_POST['fechaC'];
    $provedor_id = $_POST['provedor'];
    $metodo_pago = $_POST['metodoPago'];
    $tipoCompra = $_POST['tipoVenta'];
    ...
    if (
        empty($compraId) || empty($fecha) || empty($provedor_id) || empty($metodo_pago) || empty($tipoCompra) ||
        empty($total) || empty($carrito)
    ) {
        ...
        $query_insert = mysqli_query($conexion, "INSERT INTO purchase 
        (id, suppliers_id, creation_date, payment_metod, type_purchase, total_amount, status) 
        VALUES ('$compraId', '$provedor_id','$fecha','$metodo_pago','$tipoCompra','$total')");
        ...
        foreach ($carrito as $producto) {
            ...
            // Insertar en `Update product`
            $query_insert2 = mysqli_query($conexion, "INSERT INTO purchasedetail 
            (product_id, purchase_id, quantity_product, unit_price) 
            VALUES ('$product_id', '$compraId', '$quantity', '$price')");

            // Actualizar la cantidad del producto comprado
            $query_update_comprado = mysqli_query($conexion, "UPDATE products SET quantity = quantity + '$quantity' WHERE id = '$product_id'");
                ...
                <div class="row">
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="row mb-2">
                                <div class="col-sm-6">
                                    <h3 class="m-0 text-dark">Nueva Compra</h3>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-3">
                                    <div class="form-group">
                                        <label style="font-family: sans-serif">No. Compra</label>
                                        <div class="input-group mb-3">
                                            <div class="input-group-prepend">
                                                <span class="input-group-text"><i class="fas fa-calendar-alt"></i></span>
                                            </div>
                                            <input type="text" required="" name="idCompra" id="idCompra" class="form-control" value="<?php echo $idCompraInicial; ?>" readonly>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group mb-3">
                                            ...
                                            <select class="form-control" name="metodoPago" id="metodoPago">
                                                <option value="efectivo">Consignación bancaria</option>
                                                <option value="datafono">Transferencia </option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="controls">
                                            <div class="input-group mb-3">
                                                ...
                                                <input id="tipoVenta" name="tipoVenta" type="text" class="form-control" value="Virtual" readonly>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label class="control-label" for="provedor">Proveedor</label>
                                        <div class="input-group mb-3">
                                            <div class="input-group-prepend">
                                                <span class="input-group-text"><i class="fas fa-copyright"></i></span>
                                            </div>
                                            <select class="form-control" id="provedor" name="provedor" class="span8 tip">
                                                <option value="">Seleccione Proveedor</option>
                                                <?php $query = mysqli_query($conexion, "select * from suppliers where status = 1");
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <div class="form-group text-center">
                                        ...
                                        <button type="submit" class="btn btn-outline-success"><span class="fas fa-save"></span> Confirmar Compra</button>
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div class="col-xl-2 col-lg-2 col-md-3 col-sm-4 col-xs-6">
                            ...
                            <div class="product-card" data-toggle="modal" data-target="#modal-<?php echo $producto['id']; ?>">
                                ...
                                <div class="card-body">
                                    ...
                                    <!-- Precios -->
                                    <div class="prices">
                                        <span class="current-price">Precio: <?php echo $precioConDescuentoFormatted; ?></span>
                                    </div>

                                    <!-- Cantidad -->
                                    <div class="quantity">
                                        <label for="quantity-<?php echo $producto['id']; ?>">Cantidad disponible</label>
                                        <?php if ($producto['stock'] == 'In Stock') { ?>
                                            <input type="number" id="quantityStock" name="quantity" style="text-align: center;" value="<?php echo $producto['quantity']; ?>" disabled>
                                        <?php } else { ?>
                                            <input type="number" id="quantityStock" name="quantity" style="color:red; text-align: center;" value="<?php echo $producto['quantity']; ?>" disabled>
                                        <?php } ?>
                                        <label for="quantity-<?php echo $producto['id']; ?>">Cantidad:</label>
                                        <?php if ($producto['stock'] == 'In Stock') { ?>
                                            <input type="number" id="quantity-<?php echo $producto['id']; ?>" name="quantity" style="text-align: center;" min="1" max="<?php echo $producto['stock']; ?> disabled" value="1">
                                        <?php } else { ?>
                                            <input type="number" id="quantity-<?php echo $producto['id']; ?>" name="quantity" style="color:red; text-align: center;" value="0">
                                        <?php } ?>
                                    </div>
                                </div>
    ...
    $(document).ready(function() {
        ...
        $('.add-to-cart').on('click', function() {
            ...
            if ($existingRow.length > 0) {
                ...
                <td class="cart-total" data-total="${productPrice}">${precioOriginalFormateado}</td>
        ...
        $('.shopping-cart-table').on('change', '.quantity-input', function() {
            ...
            var precioConDescuentoAplicado = productPrice * quantity;
        ...
        function calculateAndUpdateTotals() {
            let total = 0;
            let ivaRate = 0; // Tasa de IVA (ejemplo: 19%)
            let discountRate = 0; // Tasa de descuento (ejemplo: 10%)
            let iva = 0;
            let descuento = 0;
        ...
        function limpiarFormulario() {
            ...
            window.location.href = '<?php echo $url; ?>'; // Redirigir al PDF
